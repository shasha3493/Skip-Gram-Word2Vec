...
def getNegativeSamples(outsideWordIdx, dataset, K):
    ...
    for k in range(K):
        # random sample token id and store if it is not equal to context token id
...
def word2vec_sgd_wrapper(word2vecModel, word2Ind, wordVectors, dataset,
                         ...
                         word2vecLossAndGradient=naiveSoftmaxLossAndGradient):
    '''
    Runs word2vec skipgram model and calculates loss and gradients
    '''
    batchsize = 50
    loss = 0.0
    ...
    for i in range(batchsize):
        ...
        centerWord, context = dataset.getRandomContext(windowSize1) # for a given window size, sample a random center words and context words
        
        # Run word2vec model to calculate loss and gradients
...
def test_sigmoid():
    ...
    Match the output of dummy input with expected output. 
    The inputs are both scalar as well as array
    """
...
def getDummyObjects():
    ...
    ["a", "b", "c", "d", "e"] - dummy words in the vocabulary
    """
    ...
    def getRandomContext(C):
        '''
        Samples a random center word and 2C outside words
        '''
        tokens = ["a", "b", "c", "d", "e"]
    ...
    random.seed(31415)
    np.random.seed(9265)
    dummy_vectors = normalizeRows(np.random.randn(10,3)) # random normalized vword vectors (2*num words in vocab, word vector length) 
    dummy_tokens = dict([("a",0), ("b",1), ("c",2),("d",3),("e",4)]) #  dictionary that maps words to their indices in the word vector list
...
def test_naiveSoftmaxLossAndGradient():
    ...
    dataset -- instance of dummy class
    dummy_vectors -- nd array of size (2*num words in vocab, word vector length), 
                     half of ot represents outside vectors and other represents center vectors
    dummy_tokens -- a dictionary that maps words to their indices in
                    the word vector list
                    
    Calculates loss and gradients using a naiveSoftmaxLoss loss function and matches them with the one
    calculates using mathematical function explained here http://ufldl.stanford.edu/tutorial/supervised/DebuggingGradientChecking/
    """
...
def test_negSamplingLossAndGradient():
    ...
    Calculates loss and gradients using a negSamplingLoss loss function and matches them with the one
    calculates using mathematical function explained here http://ufldl.stanford.edu/tutorial/supervised/DebuggingGradientChecking/
    """
    dataset, dummy_vectors, dummy_tokens = getDummyObjects()
...
def test_skipgram():
    """ Test skip-gram with naiveSoftmaxLossAndGradient and negSamplingLossAndGradient
    ...
    # Compares the gradient calculated from loss fn with the ones calculated from mathematical formula 
    gradcheck_naive(lambda vec: word2vec_sgd_wrapper(
        skipgram, dummy_tokens, vec, dataset, 5, naiveSoftmaxLossAndGradient),
        dummy_vectors, "naiveSoftmaxLossAndGradient Gradient")    
    # Compares the gradient calculated from loss fn with the ground truth (calcualted ahead of time)
